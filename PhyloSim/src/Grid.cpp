...
Landscape::Landscape(int xsize, int ysize, int type, bool neutral, bool dd, bool env, bool mort, bool repro,
                     unsigned int runs, double specRate, int dispersalCutoff, int DensityCutoff,
                     unsigned int mortalityStrength, double envStrength, double compStrength, int fission,
...
void GlobalEnvironment::reproduce(unsigned int generation) {
  ...
  // Density dependence and / or Environmental dependence
      ...
      for (int kernel_x = 0; kernel_x < m_Xdimensions; kernel_x++) {
        for (int kernel_y = 0; kernel_y < m_Ydimensions; kernel_y++) {
          weights[array_length] =
              m_Individuals[kernel_x][kernel_y].getFitness(m_Environment[kernel_x * m_Ydimensions + kernel_y].first,
                                                           m_Env, m_DD, generation, m_redQueenStrength, m_redQueen);
    ...
    while (numberDeath < numberOfRuns) {
      ...
      // TODO: old comment was: 'report birth and evolve is automatic' -> this is wrong, neither evolve nor report birth
      // are automatic. Evolve is below, what about report birth
      m_Individuals[x_coordinate][y_coordinate].m_X_coordinate = x_coordinate;
      m_Individuals[x_coordinate][y_coordinate].m_Y_coordinate = y_coordinate;
      ...
      //         m_Individuals[x_coordinate][y_coordinate].m_LocalDensity = spread + std::abs(averageCompetitionTrait -
      ...
      //           for(int y = 0 ; y <  m_Ydimensions ; y++)
      //           {
      //              m_Individuals[x][y].m_LocalDensity = spread + std::abs(averageCompetitionTrait -
      ...
      if (m_reproduction) {
        ...
        //
        //              double envFitnessParent = 1.2 * exp(-0.5 * pow((m_Environment[x_coordinate * m_Ydimensions +
        //              y_coordinate].first - m_Individuals[x_coordinate][y_coordinate].m_Mean) /
        //              m_Individuals[x_coordinate][y_coordinate].m_Variance, 2.0));
        //              //						double envFitnessPropagule = (1.0 /
        //              (individuals[kernel_x][kernel_y].variance * sqrt(2.0 * 3.147))) * exp(-0.5 *
        //              pow((environment[x_coordinate][y_coordinate].first - individuals[kernel_x][kernel_y].mean) /
        //              individuals[kernel_x][kernel_y].variance, 2.0)); // environmental influence ! newWeight =
        //              envFitnessParent  + (DBL_MIN*100.0) ; //weights plus base value
        //
        //              //						double envFitnessParent = (1.0 /
        //              (individuals[x_coordinate][y_coordinate].variance * sqrt(2.0 * 3.147))) * exp(-0.5 *
        //              pow((environment[x_coordinate][y_coordinate].first -
        //              individuals[x_coordinate][y_coordinate].mean) /
        //              individuals[x_coordinate][y_coordinate].variance, 2.0)); // environmental influence !
        //              //						double envFitnessPropagule = (1.0 /
        //              (individuals[x_coordinate][y_coordinate].variance * sqrt(2.0 * 3.147))) * exp(-0.5 *
        //              pow((environment[x_coordinate][y_coordinate].first -
        //              individuals[x_coordinate][y_coordinate].mean) /
        ...
        //                          {
        //                             relatedness += std::abs(m_Individuals[focus_x][focus_y].m_CompetitionMarker -
        ...
        //                    if(m_Env)
        //                    {
        //                       double envFitnessParent = 1.2 * exp((-0.5 * (m_Environment[focus_x * m_Ydimensions +
        //                       focus_y].first - m_Individuals[focus_x][focus_y].m_Mean) /
        //                       m_Individuals[focus_x][focus_y].m_Variance * (m_Environment[focus_x * m_Ydimensions +
        //                       focus_y].first - m_Individuals[focus_x][focus_y].m_Mean) /
        //                       m_Individuals[focus_x][focus_y].m_Variance)); // environmental influence !
        //                       //								double
        //                       envFitnessPropagule = (1.0 / (individuals[focus_x][focus_y].variance * sqrt(2.0
        //                       * 3.147))) * exp((-0.5 * (environment[x_coordinate][y_coordinate].first -
        //                       individuals[focus_x][focus_y].mean) / individuals[focus_x][focus_y].variance *
        //                       (environment[x_coordinate][y_coordinate].first - individuals[focus_x][focus_y].mean) /
        //                       individuals[focus_x][focus_y].variance)); // environmental influence !
        //
        //                       seedSum -= weights[focus_x*m_Ydimensions + focus_y ];
        //                       weights[focus_x*m_Ydimensions + focus_y ] = envFitnessParent  *
        //                       m_Individuals[focus_x][focus_y].m_LocalDensity + (DBL_MIN*100.0); seedSum +=
        ...
        //              }
        //
        //              unsigned int start =  ((x_coordinate -m_DensityCutoff + m_Xdimensions) % m_Xdimensions) *
        //              m_Ydimensions + ((y_coordinate - m_DensityCutoff + m_Ydimensions) % m_Ydimensions) ; unsigned
        //              int end = m_LandscapeSize;
        //
        ...
        //                 //						std::cout << seedSum << " : " << cumWeights[kk]
        //                 << " : " << kk << " : " << array_length-1 <<  '\n';
        //              }
        //              //					std::cout << seedSum << " : " <<
        //              cumWeights[array_length-1] << " : " << array_length-1  << " : " << start  << " : " << end <<
        //              '\n';
        //           }
        //
      } // END if (m_reproduction)
...
void LocalEnvironment::reproduce(unsigned int generation) {
  ...
  while (numberDeath < numberOfRuns) {
    ...
    if (m_mortality && event % m_mortalityStrength != 0) {
      double weight = m_Individuals[x_coordinate][y_coordinate].getFitness(
          m_Environment[x_coordinate * m_Ydimensions + y_coordinate].first, m_Env, m_DD, generation, m_redQueenStrength,
          m_redQueen);
      double chanceOfDeath = m_RandomGenerator.randomDouble(0.0, 1.0);
      if (weight > chanceOfDeath)
        continue;
        ...
        if (!(kernel_x == x_coordinate && kernel_y == y_coordinate)) {
          ...
          if (m_reproduction) {
            weights[array_length] = m_Individuals[kernel_x][kernel_y].getSeedsTo(
                ...
                m_DD, generation, m_redQueenStrength, m_redQueen);
          } else if (m_mortality) {
    ...
    m_Individuals[x_coordinate][y_coordinate] =
        m_Individuals[x_parent][y_parent]; // overloaded operator, deep copy ! // report birth and evolve is automatic
    m_Individuals[x_coordinate][y_coordinate].m_X_coordinate = x_coordinate;
    m_Individuals[x_coordinate][y_coordinate].m_Y_coordinate = y_coordinate;
